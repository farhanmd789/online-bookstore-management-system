...
public class BookServiceImpl implements BookService {
    ...
    private static final String searchBooksQuery = "SELECT * FROM " + BooksDBConstants.TABLE_BOOK
            + " WHERE LOWER(" + BooksDBConstants.COLUMN_NAME + ") LIKE ? OR LOWER("
            + BooksDBConstants.COLUMN_AUTHOR + ") LIKE ?";
    ...
    @Override
    public List<Book> searchBooks(String query) throws StoreException {
        List<Book> books = new ArrayList<Book>();
        Connection con = DBUtil.getConnection();
        try {
            PreparedStatement ps = con.prepareStatement(searchBooksQuery);
    ...
    @Override
    public String getNextBookId() throws StoreException {
        List<Book> books = getAllBooks();
        long maxId = 0;
        for (Book book : books) {
            try {
                long id = Long.parseLong(book.getBarcode());
                if (id > maxId) maxId = id;
            } catch (NumberFormatException e) {
                // skip non-numeric barcodes (e.g. old UUID values)
            }
        }
